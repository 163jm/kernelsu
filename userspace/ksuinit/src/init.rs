...
fn setup_kmsg() {
    // Logging is globally silenced: ksuinit produces zero log output.
    // We keep the /dev/kmsg node creation for kernel compatibility but
    // never initialize the kmsg logger, so all log::* calls are dropped.
